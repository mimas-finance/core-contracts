...
contract ComptrollerVXStorage is UnitrollerAdminStorage {
     ...
     */
    mapping(address => MmToken[]) public accountAssets;
    ...
    struct Market {
        ...
        /// @notice Whether or not this market receives MIMAS
        bool isMimed;
    }

    /**
     * @notice Official mapping of mmTokens -> Market metadata
    ...
    MmToken[] public allMarkets;

    // @notice The borrowCapGuardian can set borrowCaps to any number for any market. Lowering the borrow cap could disable borrowing on the given market.
    address public borrowCapGuardian;

    // @notice Borrow caps enforced by borrowAllowed for each mmToken address. Defaults to zero which corresponds to unlimited borrowing.
    ...
    /// @notice The MIMAS/CRO market borrow state for each market
    mapping(uint8 =>mapping(address => RewardMarketState)) public rewardBorrowState;

    /// @notice The MIMAS/CRO borrow index for each market for each supplier as of the last time they accrued reward
    mapping(uint8 => mapping(address => mapping(address => uint))) public rewardSupplierIndex;

    /// @notice The MIMAS/CRO borrow index for each market for each borrower as of the last time they accrued reward
